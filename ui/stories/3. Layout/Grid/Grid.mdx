...
### 3. Center the grid
- Add the `data-grid="centered"` attribute in the HTML. This will only work if the grid does not use the full width of its container.
...
- Simply define a `.grid` parent with 2 direct children, and add `data-grid="asymmetric"` attribute.

### 2. Override asymmetric column widths
- For example, use `data-grid="asymmetric-75-25"` or `data-grid="asymmetric-25-75"`.
