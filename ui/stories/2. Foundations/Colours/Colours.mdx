...
> NOTE: The Storybook UI and stories switch to dark mode, but docs pages do not. There is a temporary workaround in `ui/public/storybook-overrides.css` to ensure docs text is legible (docs pages are rendered in light mode).
...
:root {
    ...
    --webui-color-primary-100: oklch(from var(--color-brand) calc(l + 0.2) c h);
}
```
