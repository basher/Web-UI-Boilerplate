...
- Fetch aborts/timeouts are handled by an [AbortController](https://developer.mozilla.org/en-US/docs/Web/API/AbortController) in `./ui/src/javascript/utils/ajax-helpers.ts`.
...
- The [Ajax loading spinner](/docs/web-components-or-custom-elements-webui-ajax-loader--docs) is an `<svg>` with an accessible `loading...` name, which will be announced in the live region.

## Required and optional HTML  or `data-` attributes

Attribute | Behaviour
--- | ---
`data-fetch-url` | Required. On the web component. URL of HTML fragment to be fetched.
`data-fetch-target` | Required. On the HTML element (i.e. button) that triggers the Fetch request. The value must match `data-fetch-container`.
