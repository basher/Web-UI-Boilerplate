...
- The CSS variable colour values are specified as a comma-separated list of individual `h`, `s` and `l` values:
```
:root { --color-brand: 270, 50%, 40%; }
```
- This enables us to use the CSS variables directly in the `hsl()` CSS function, providing access to the `alpha` channel for `opacity`:
```
