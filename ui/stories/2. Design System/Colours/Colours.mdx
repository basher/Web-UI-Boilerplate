...
> See [this OKLCH article](https://evilmartians.com/chronicles/oklch-in-css-why-quit-rgb-hsl) which explains why OKLCH is better than HSL. Also see this [OKLCH color picker](https://oklch.com/#70,0.1,293,100) tool.
...
```
- Semantic tokens are declared like this, with access to the `alpha` channel for `opacity`:
```
:root {
    --color-primary-500: var(--webui-color-brand);
    --color-primary-100: oklch(from var(--webui-color-brand) l c h / 30%);
}
```
