...
- Any "grouped" form fields (e.g. `<input type="radio">`) should be enclosed in a `<fieldset>` with an appropriate `<legend>` to provide additional context for screen readers.
- Required fields need a `required` attribute on the inputs, etc. There is no need to add a visual indicator.
- Optional fields should be identified as such in the `<label>`.
